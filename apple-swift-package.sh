...
APPLE_SWIFT_BUILD=apple-swift-${REL}-${OS_VER}
...
FILENAME=$SWIFT_VERSION-$SWIFT_PLATFORM
...
rsync -Hav $FILENAME/usr/  ${APPLE_SWIFT_BUILD}/usr/local
export RELEASE=$REL-$SWIFT_PLATFORM BUILD
...
if [ "$skip_repo" != "yes" ] ; then
    # Move to repo
    cp $APPLE_SWIFT_BUILD-$BUILD.deb /usr/local/debs/amd64/
